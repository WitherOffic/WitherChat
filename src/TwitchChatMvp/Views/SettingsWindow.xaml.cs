...
public partial class SettingsWindow : Window
{
        ...
        string readOnlyChannel = "")
    {
        ...
        Loaded += (_, _) => AnimationService.AnimateWindowIn(this);
    ...
    private async void Save_Click(object sender, RoutedEventArgs e)
    {
        CommitNumericValues();
        Settings.Normalize();
        await CompleteDialogAsync(true);
    }

    private async void Cancel_Click(object sender, RoutedEventArgs e)
    {
        await CompleteDialogAsync(false);
    }
    ...
    private async void ChangeWatchChannel_Click(object sender, RoutedEventArgs e)
    {
        CommitNumericValues();
        Settings.Normalize();
    ...
    private async Task CompleteDialogAsync(bool? result)
    {
        ...
        await AnimationService.AnimateWindowCloseAsync(this).ConfigureAwait(true);
        _allowClose = true;
        DialogResult = result;
    }
    ...
    {
        if (HandleNumericKey(e, () => AdjustFontSize(1), () => AdjustFontSize(-1)))
        {
            return;
        }
    }
    ...
    {
        if (HandleNumericKey(e, () => AdjustMessageLimit(100), () => AdjustMessageLimit(-100)))
        {
            return;
        }
    }
    ...
    private bool HandleNumericKey(KeyEventArgs e, Action increase, Action decrease)
    {
        switch (e.Key)
        {
            case Key.Up:
